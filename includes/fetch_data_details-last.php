    ...
    foreach ($this->results as $items) {
        ...
        if ($pw_detail_view == "yes") {
            if ($new_order) {
                ...
                $has_additional_data = maybe_unserialize(wc_get_order_item_meta(  $item_id, '_ywapo_meta_data'));
                ...
                $datatable_value .= $items->billing_name;
                $datatable_value .= ("</td>");
                $order_customer_name = $items->billing_name;
                ...
                $datatable_value .= $items->billing_phone;
                $datatable_value .= ("</td>");
                $order_customer_phone = $items->billing_phone;
                ...
                if ($order_status_changed != '') {
                    $datatable_value .= date($date_format, $order_status_changed);
                    $order_mdate = date($date_format, $order_status_changed);
                } else {
                    $date_modified = get_post_field('post_modified', $order_id);;
                    $datatable_value .= date($date_format, strtotime($date_modified));
                    $order_mdate = date($date_format, strtotime($date_modified));
                }
                $datatable_value .= ("</td>");
                ...
                $datatable_value .= date($date_format, strtotime($items->order_date));
                $datatable_value .= ("</td>");
                $order_cdate = date($date_format, strtotime($items->order_date));
...
//				$pw_table_value			= $varation_string[$order_item_id]['varation_string'];
                ...
                $datatable_value .= ("<td class='tarik-3' style='" . $display_class . "'>");
				if (wc_get_order_item_meta( $items->order_item_id, '_ywcars_item_refunded') == 'yes') {
					$pw_table_value = -1 * abs($pw_table_value);
				}
                $datatable_value .= $this->price($pw_table_value,
