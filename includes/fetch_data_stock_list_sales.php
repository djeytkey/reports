...
if ($file_used == "sql_table") {
	...
	if (!is_numeric($pw_txt_min_stock))
		$pw_txt_min_stock = 0;
	if (!is_numeric($pw_txt_max_stock))
		$pw_txt_max_stock = 0;
	...
	$sql = "SELECT $sql_columns
			FROM $current_lng_join $sql_joins $pw_sku_number_join $pw_product_stock_join
			...
			$pw_product_name_conditoin $pw_product_id_conditoin
			...
			$pw_zero_sold_conditoin $pw_stock_status_conditoin $current_lng_cond GROUP BY product_id";
	// $sql = "SELECT $sql_columns
	...
	if ($pw_basic_column == 'yes') {
		...
		$columns = array(
			...
			array('lable' => esc_html__('Stock', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
			array('lable' => esc_html__('Variations', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
			...
			// array('lable' => esc_html__('Downloadable', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'hide'),
			// array('lable' => esc_html__('Virtual', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'hide'),
			// array('lable' => esc_html__('Manage Stock', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'hide'),
			// array('lable' => esc_html__('Backorders', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'hide'),
	...
	foreach ($this->results as $items) {
		...
		if ($pw_basic_column == 'yes') {
			...
			if ($this->pw_get_pt_product_id($items->product_id) == 'Variable')
				$product_type = "Variable Product";
			else
				$product_type = $this->pw_get_pt_product_id($items->product_id);
			...
			if ($this->table_cols[$index_cols++]['status'] == 'hide')
				$display_class = 'display:none';
			$datatable_value .= ("<td style='" . $display_class . "'>");
			$datatable_value .= $this->pw_get_pt_product_id($items->product_id) != 'Variable' ? $this->price($regular_price) : "";
			$datatable_value .= ("</td>");
			...
			if ($this->table_cols[$index_cols++]['status'] == 'hide')
				$display_class = 'display:none';
			$datatable_value .= ("<td style='" . $display_class . "'>");
			$datatable_value .= $this->pw_get_pt_product_id($items->product_id) != 'Variable' ? $this->price($sale_price) : "";
			$datatable_value .= ("</td>");
		...
		if ($this->pw_get_pt_product_id($items->product_id) == 'Variable') {
			...
			$to_display = '';
			foreach ($children_ids as $key => $variation) {
				$datatable_value .= ("<tr>");
				//SKU
				$display_class = '';
				if ($this->table_cols[$index_cols++]['status'] == 'hide')
				...
				//Variations
				$display_class = '';
				$variationObject = new WC_Product_Variation($children_ids[$key]);
				$variationName = implode(" / ", $variationObject->get_variation_attributes());
				...
				$regular_price = get_post_meta( $children_ids[$key], '_regular_price', true );
				$sale_price = get_post_meta( $children_ids[$key], '_sale_price', true );
				...
				if ($this->table_cols[$index_cols++]['status'] == 'hide')
					$display_class = 'display:none';
				$datatable_value .= ("<td style='" . $display_class . "'>");
				$datatable_value .= $this->price($regular_price, array("currency" => "no curreny"), 'multi_currency');
				$datatable_value .= ("</td>");
				...
				if ($this->table_cols[$index_cols++]['status'] == 'hide')
					$display_class = 'display:none';
				$datatable_value .= ("<td style='" . $display_class . "'>");
				$datatable_value .= $sale_price!='' ? $this->price($sale_price, array("currency" => "no curreny"), 'multi_currency') : "--";
				$datatable_value .= ("</td>");
	...
	?>
	<form
		class='alldetails search_form_report'
		action=''
		method='post'
	>
		<input
		...
		<div class="row">
			...
			<div class="col-md-6">
				...
				<input
					name="pw_name_of_product"
					type="text"
					class="pw_name_of_product"
				/>
			</div>
			...
			<div class="col-md-6">
				...
				<span class="awr-form-icon"><i class="fa fa-cog"></i></span>
				<input
					name="pw_stock_product"
					type="text"
					class="pw_stock_product"
				/>
			</div>
			...
			<div class="col-md-6">
				...
				<span class="awr-form-icon"><i class="fa fa-check"></i></span>
				<select
					name="pw_sub_product_type"
					id="pw_sub_product_type"
				>
			...
			<div class="col-md-6">
				...
				<span class="awr-form-icon"><i class="fa fa-check"></i></span>
				<select
					name="pw_status_of_stock"
					id="pw_status_of_stock"
					class="pw_status_of_stock"
				>
			...
			<div class="col-md-6">
				...
				<select
					name="pw_stock_manage"
					id="pw_stock_manage"
					class="pw_stock_manage"
				>
					<option value="-1">All</option>
				...
				>
					...
					>
						...
						if ($this->get_form_element_permission('pw_product_id') && ((!is_array($permission_value)) || (is_array($permission_value) && in_array('all', $permission_value)))) {
							?>
							<option value="-1">
								<?php _e('Select All', __PW_REPORT_WCREPORT_TEXTDOMAIN__); ?>
							</option>
							<?php
						}
						?>
			...
			<div class="col-md-6">
				...
				<input
					type="checkbox"
					name="pw_general_cols"
					class="pw_general_cols"
					value="yes"
				>
			</div>
		...
		<div class="col-md-12">
			...
			<input
				type="hidden"
				name="list_parent_category"
				value=""
			>
			<input
				type="hidden"
				name="group_by_parent_cat"
				value="0"
			>
			...
			<input
				...
				name="pw_hide_os"
				id="pw_hide_os"
				value="<?php echo $pw_hide_os; ?>"
			/>
			<input
				type="hidden"
				name="pw_id_order_status[]"
				id="pw_id_order_status"
				value="-1"
			>
			<input
				type="hidden"
				name="pw_orders_status[]"
				id="order_status"
			...
			><i class="fa fa-search"></i> <span>
					<?php echo esc_html__('Search', __PW_REPORT_WCREPORT_TEXTDOMAIN__); ?>
				</span></button>
			<button
				type="button"
				value="Reset"
				class="button-secondary form_reset_btn"
			><i class="fa fa-reply"></i><span>
					<?php echo esc_html__('Reset Form', __PW_REPORT_WCREPORT_TEXTDOMAIN__); ?>
				</span></button>
		</div>
