...
if ($file_used == "sql_table") {
    ...
    if ($pw_detail_view == "yes") {
        ...
        $columns = array(
            ...
            array('lable' => esc_html__('القيمة المضافة', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'currency'),
            ...
            array('lable' => esc_html__('ملاحظات العميل', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
        );
    ...
    foreach ($this->results as $items) {
        ...
        if ($pw_detail_view == "yes") {
            if ($new_order) {
                ...
                $order_invoice_id = '';
				$order_invoice_id = get_post_meta($items->order_id, '_wcpdf_invoice_number', true);

                if ($order_invoice_id) {
                    $datatable_value .= '<a target="_blank" href="' . admin_url() . 'admin-ajax.php?action=generate_wpo_wcpdf&document_type=invoice&order_ids=' . $items->order_id . '&access_key=40fe906e1e"><i class="fa fa-download button "></i></a>';
                } else {
                    $datatable_value .= '---';
                }
                ...
                if( $notes ) {
                    $datatable_value .= ("<ul style='list-style: disc; direction: rtl; text-align: right;'>");
                    foreach( $notes as $note ) {
                        if ($note->added_by != "system") {
                            if( $note->customer_note ) {
                                $datatable_value .= ("<li class='order-customer-note'><span style='position: relative; right: -5px;'>");
                                $datatable_value .= $note->content;
                                $datatable_value .= ("</span></li>");
                            }
                        }
                    }
                    $datatable_value .= ("</ul>");
        		...
        		if ($value_unserialized) {
        			...
        			for ($x = 0; $x < count($value_unserialized); $x++) {
        				...
        				if ($addon_id[0] == "product") {
        					...
        					$price_to_deduct += wc_get_price_excluding_tax( $product );
        				}
        			}
                ...
                // $datatable_value .= $this->price($pw_table_value,
                    // array("currency" => "no currency", "order_id" => $items->order_id), 'multi_currency');
                ...
                //TOTAL TAX
                $display_class = '';
                $pw_table_value = wc_get_order_item_meta( $items->order_item_id, '_line_subtotal_tax');
                ...
                if ($has_serialized_data && !$is_individually_sold) {
                    for ($x = 0; $x < count($has_serialized_data); $x++) {
                        ...
                        if ($addon_id[0] == "product") {
                            ...
                            $datatable_value .= number_format ( $product_price, 4, ",", "" );
                            ...
                            $datatable_value .= ("<td style='" . $display_class . "'>");
                            $datatable_value .= number_format ( $total_price, 4, ",", "" );
                            ...
                            $datatable_value .= ("<td style='" . $display_class . "'>");
                            $datatable_value .= number_format ( $related_product_tax, 4, ",", "" );
                            // $datatable_value .= $this->price($related_product_tax, array("currency" => "no currency", "order_id" => $items->order_id), 'multi_currency');				
                            $datatable_value .= ("</td>");
					...
					$is_gift = wc_get_order_item_meta( $items->order_item_id, '_fgf_gift_product');
					if (!empty($is_gift)) {
						$pw_table_value = isset($items->product_rate) ? $items->product_rate : 0;
						$pw_table_value = $pw_table_value == 0 ? $pw_null_val : $pw_table_value;
					}
