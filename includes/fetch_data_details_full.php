...
//print_r($ff_columns);
    ...
    if ($pw_detail_view == "yes") {
        ...
        $columns = array(
            ...
            array('lable' => esc_html__('المحفظة / الخصم', __PW_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'currency'),
            //CUSTOM WORK - 4187
...
//                $state      	= $this->pw_get_woo_bsn($order->billing_country,$items->billing_state);
                ...
                $datatable_value .= ("<td style='" . $display_class . "'>");
                $datatable_value .= $this->price($fee_total, array("currency" => "no currency", "order_id" => $items->order_id), 'multi_currency');
                $datatable_value .= ("</td>");
